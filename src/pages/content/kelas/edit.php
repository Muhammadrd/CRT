...
<body>
    ...
    <div class="container" style="width: fit-content; margin-left:20px; width:500px;">
        ...
        <form action="/kelas/<?= $mahasiswa['id']; ?>/update" method="post">
            ...
            <div style="text-align: left;"><label class="form-check-label" for="">Mata Kuliah</label><br></div>
            <div class="form-check">

                <?php foreach ($crud_matakuliah->items as $key => $value) { ?>
                    <input class="form-check-input" type='checkbox' class="form-control" name='mk[]' value="<?= $value['id'] ?>" <?= in_array($value['id'], $ar_matkul) ? 'checked' : '' ?> /> <?= $value['mata_kuliah'] ?><br>
                <?php } ?>
            </div>
